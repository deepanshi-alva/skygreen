...
export default function LeftInputPanel({ onResults }: LeftInputPanelProps) {
    ...
    const [filteredStates, setFilteredStates] = useState<StateType[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/calculator/estimate`, {
                method: "POST",
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    {selectedState ? (
                        <div className="flex gap-4">
                            ...
                            <label className={selectedState.rwa_enabled ? "" : "opacity-50 cursor-not-allowed"}>
                                <input
                                    type="radio"
                                    name="mode"
                                    value="rwa"
                                    ...
                                    onChange={handleChange}
                                    disabled={!selectedState.rwa_enabled}   // 🚀 disables RWA if not enabled
                                    className="mr-2"
                                />
                                RWA / GHS
                            </label>
                ...
                {formData.mode === "residential" ? (
                    ...
                    <>
                        ...
                        <div>
                            <label className="block mb-1">Total Sanctioned Load of Society (kW)</label>
                            <input
                                type="number"
                        ...
                        <div>
                            <label className="block mb-1">Total Monthly Units Consumed (kWh)</label>
                            <input
                                type="number"
                                name="societyUnits"
